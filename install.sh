...
LOG="Install-Logs/01-Sway-Install-Scripts-$(date +%d-%H%M%S).log"
...
whiptail --title "KooL-style Fedora-Sway (2025) Install Script" \
    --msgbox "Welcome to KooL-style Fedora-Sway (2025) Install Script!!!\n\n\
...
options_command+=(
    ...
    "wlr_portal" "Install xdg-desktop-portal-wlr (screen share)?" "OFF"
    "zsh" "Install zsh shell with Oh-My-Zsh?" "OFF"
    "pokemon" "Add Pokemon color scripts to your terminal?" "OFF"
    "rog" "Are you installing on Asus ROG laptops?" "OFF"
    "dots" "Download and install pre-configured KooL-style Sway dotfiles?" "OFF"
)
...
while true; do
    ...
    if [[ "$dots_selected" == "OFF" ]]; then
        ...
        if ! whiptail --title "KooL-style Sway Dot Files" --yesno \
        "You have not selected to install the pre-configured KooL-style Sway dotfiles.\n\nKindly NOTE that if you proceed without Dots, Sway will start with default vanilla Sway configuration and I won't be able to give you support.\n\nWould you like to continue install without KooL-style Sway Dots or return to choices/options?" \
    ...
    echo "👌 ${OK} You confirmed your choices. Proceeding with ${SKY_BLUE}KooL 🇵🇭 Sway Installation...${RESET}" | tee -a "$LOG"
    break
done
...
for option in "${options[@]}"; do
    case "$option" in
        ...
        rog)
            ...
            ;;
        dots)
            echo "${INFO} Installing pre-configured ${SKY_BLUE}KooL-style Sway dotfiles...${RESET}" | tee -a "$LOG"
...
if rpm -q sway &> /dev/null; then
    ...
    printf "${SKY_BLUE}Thank you${RESET} 🫰 for using 🇵🇭 ${MAGENTA}KooL-style Sway Dots${RESET}. ${YELLOW}Enjoy and Have a good day!${RESET}"
    printf "\n%.0s" {1..2}

    printf "\n${NOTE} You can start Sway by typing ${SKY_BLUE}sway${RESET} (IF SDDM is not installed).\n"
